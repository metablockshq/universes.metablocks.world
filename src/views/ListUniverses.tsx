...
function NoUniverses() {
  return <div className="mt-10 w-2/3 lg:w-2/6 mx-auto">
    <Placard
      imgSrc={woman}
...
function Universes({ data, error }) {
  return <div className="mt-10 mx-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
    {data &&
      !error &&
      data.map(
        (u) => (
...
function CrawlInfo({ data }) {
  return <div className="mt-4">
    <Callout icon={"time"} intent={Intent.PRIMARY}>
      Last crawled {formatDistanceToNow(
        fromUnixTime(data.timestamp),
      )} ago.

      It takes about an hour for your newly created universe to reflect on the index.
    </Callout>
  </div>
}
...
function ListUniverses() {
  ...
  return (
    <>
      ...
      {!lastCrawledError && lastCrawledData && <CrawlInfo data={lastCrawledData} />}
